...
import { GENERATE_IMAGE } from '@/graphql/mutations/generate-image';
import { useSession } from 'next-auth/react';

const DEFAULT_SYSTEM_PROMPT = `Developer: You are an expert image generator. Create professional marketing visuals using the provided reference images.

Guidelines:
1. Create high-quality, professional marketing imagery that complements any provided reference images.
2. Ensure the image works well for web display at a 1536x1024 resolution.
3. Use colors and design elements that work harmoniously with any provided reference images.
4. Output the generated image in WebP format for optimal web performance.
5. If reference images are provided, use them as visual inspiration for style, colors, and theme.
6. If no reference images are provided, create original content based solely on the text prompt given.
7. Always maintain professional quality; ensure the composition is suitable for marketing purposes.
8. Create visually appealing and cohesive designs appropriate for digital marketing campaigns.

Logo Placement Instructions:
- If the FIRST reference image is provided, it represents the advertiser icon and MUST be placed in the BOTTOM RIGHT corner of the generated image.
- If both SECOND and THIRD reference images are provided, both represent the site logo: the SECOND is for light backgrounds, and the THIRD is for dark backgrounds. Place either the SECOND or THIRD reference image in the TOP LEFT corner of the generated image, selecting the appropriate logo based on the primary background color for optimal visibility.
- Logos should be sized appropriately (approximately 80–120px in the largest dimension) and maintain aspect ratio.
- Add subtle drop shadows or backgrounds to logos as needed for visibility.
- Any additional reference images beyond the first three should be used exclusively for style and theme inspiration and not placed directly in the generated image.

## Output Format
The tool must output a JSON object with the following structure:
{
  "image_webp_base64": "<base64-encoded WebP image>",
  "image_metadata": {
    "width": 1536,
    "height": 1024,
    "file_type": "webp",
    "logos_used": {
      "advertiser_icon_position": "bottom_right" (if used),
      "site_logo_position": "top_left",  
      "site_logo_variant": "light" or "dark" (depending on which logo was used) 
    },
    "reference_images_used": [<list of indices of reference images used as inspiration>]
  },
  "errors": [<array of error messages, or empty array if no errors>]
}

Example output:
{
  "image_webp_base64": "<base64 data>",
  "image_metadata": {
    "width": 1536,
    "height": 1024,
    "file_type": "webp",
    "logos_used": {
      "advertiser_icon_position": "bottom_right",
      "site_logo_position": "top_left",
      "site_logo_variant": "light"
    },
    "reference_images_used": [0,2,3]
  },
  "errors": []
}

If reference images are missing or insufficient for required logo placement, include an appropriate error message in the "errors" array.`;
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <Button
            ...
            variant="outline"
            size="sm"
            onClick={() => append({ url: '', type: undefined })}
            disabled={isLoading}
              ...
              <div key={field.id} className="space-y-2">
                ...
                <div className="ml-2">
                  <ControlledSelect
                    ...
                    options={[
                      {
                        value: 'advertiser_icon',
                        label: 'Advertiser Icon (Bottom Right)',
                      },
        ...
        {fields.length === 0 && (
          <div className={`text-sm ${TEXT_COLORS.muted}`}>
            Add up to 5 reference images to help guide the AI generation.
            Specify image types for precise placement: advertiser icons (bottom
            right), site logos (top left), or style inspiration.
          </div>
        )}
      </div>
...
export const AdminImageGenerationPlaygroundPage = () => {
  ...
  const onSubmit = async (data: PromptFormData) => {
    ...
    try {
      ...
      const generatedImageResult: GeneratedImageResult = {
        url:
          imageData.imageUrl ||
          `data:image/jpeg;base64,${imageData.imageBase64}`,
        revisedPrompt: imageData.prompt,
